describe('testing the header', () =>{
    ...
    it('testing the logo link', ()=>{
        cy.get('#header').find('.container').find('.row').find('.col-sm-4')
    ...
    it('testing the headers buttons',()=>{
        ...
        let pages = ['/', '/products', '/view_cart', '/login', '/test_cases', '/api_list',
            'https://www.youtube.com/c/AutomationExercise', '/contact_us'
        ]
        cy.get('#header').find('.container').find('.row').find('.col-sm-8')
            .find('ul>li>a').each(($a, index)=>{
